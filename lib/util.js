...
// Sort middlewares by priority without changing order of middlewares
// without specified priority.
exports.rearrangeMiddleware = function rearrangeMiddleware(server) {
  // middleware stack located at server._router for Express4
  if (server._router) server = server._router;

  var prioritySpecified = (server.stack || []).filter(function(mw) {
    return (typeof mw.handle.middlewarePriority !== "undefined");
  });

  var priorityUnspecified = (server.stack || []).filter(function(mw) {
    return (typeof mw.handle.middlewarePriority === "undefined");
  });

  prioritySpecified = prioritySpecified.sortBy(function(mw) {
    return mw.handle.middlewarePriority;
  });

  server.stack = prioritySpecified.concat(priorityUnspecified);
}
